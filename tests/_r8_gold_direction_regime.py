"""
Round 8 Gold Direction + Regime Analysis (arbiter-gold | 2026-04-19)

Run this to generate measured per-direction + per-regime tables that replace
the inferred estimates in logs/round8/gold_direction_regime.log.

Usage:
    python tests/_r8_gold_direction_regime.py

Output:
    logs/round8/gold_direction_regime_live.log
"""
...
SYMBOL, INTERVAL, PERIOD, CAPITAL, RISK = "GC=F", "1h", "10y", 10_000.0, 0.01
LOG = Path(__file__).resolve().parent.parent / "logs" / "round8" / "gold_direction_regime_live.log"
...
p("=" * 72)
p("  ARBITER-GOLD R8 | Gold Direction + Regime | LIVE | 2026-04-19")
p("=" * 72)

p('Fetching OANDA Gold 10Y...')
