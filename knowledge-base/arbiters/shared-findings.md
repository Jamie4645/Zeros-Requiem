...
### 2026-07-04 | arbiter-gold | MPB + VTC (KB-93) N3 KILL is DESERVED — flat-$ cost model masks a genuine year-over-year artifact
**Hypothesis:** Check whether the N3 KILL of both fresh candidates (MPB, VTC) is a harness bug, and whether the reported edge concentrates by direction/session/era.
**Test:** Read `logs/fresh_strategy/gauntlet_{mpb,vtc}.json/.log`; reproduced the exact CENTER cell (MPB ER_MIN=0.30/RR=1.8, VTC THRUST_ATR=1.5/CLV=0.80) via the same `analysis.real_trades.ztt_sim.simulate` call used by the gauntlet, and broke the 3,753 (MPB) / 3,225 (VTC) trades down by direction, UTC hour, and calendar year.
**Result:**
1. KILL is deserved, not a bug. Mean R/trade: MPB −0.298R, VTC −0.294R, over 3,753 / 3,225 trades — WR ~36%, exit mix ~60% SL / 33% TP / 7-9% timeout for both. Grid-avg PF 0.44 (MPB) / 0.41 (VTC), 0% of 9 cells above PF 1.0 (plateau requirement was ≥70%). No cell in either grid is close to breakeven.
2. Long/short: MPB long −0.286R/PF 0.481 vs short −0.311R/PF 0.410; VTC long −0.259R/PF 0.412 vs short −0.340R/PF 0.392. Direction is NOT the failure mode — both sides are decisively negative; the mild long>short tilt is consistent with Gold's known structural long bias (cf. SBRS long PF 2.84 vs short 2.40) but far too small to rescue either candidate.
3. Session: worst UTC entry hour for BOTH independently-built candidates is 20:00 (MPB meanR −1.70, n=79; VTC meanR −0.60, n=53) — this is the NY-close hour and directly reproduces the pre-existing CLAUDE.md finding "16-20 GMT session loses money" on two brand-new, unrelated mechanisms. Best hours cluster in the Asian session (02:00-08:00 UTC) for both. This is a real, recurring Gold microstructure effect, not candidate-specific noise.
4. Era: mean R is negative in literally every calendar year 2016-2025 for both candidates (e.g. MPB 2016 −0.420R → 2025 −0.089R; VTC 2016 −0.468R → 2025 −0.064R, with VTC's partial 2026 the only slightly-positive slice at +0.068R/n=144). The apparent "improvement" is NOT a regime effect: `src/regimes/ztt_costs.py` charges a FLAT DOLLAR one-way cost (spread $0.40-1.50 + $0.20 slip) that does not scale with the gold price, while SL distance is defined as a % of entry (0.15-0.45%). Mean Gold close rose from $1,280 (2016) to $4,767 (2026, partial) — a 3.7x increase — so the same fixed-$ cost has shrunk from ~65% of a min-SL trade's risk budget in 2016 to ~18% in 2026. The year-over-year "improvement" is a cost-model artifact of a % stop against a rising nominal price, not an emerging edge. No cell/year should be read as supporting a "works in the current regime" narrative.
**Evidence:** `logs/fresh_strategy/gauntlet_mpb.json`, `gauntlet_vtc.json`, `gauntlet_mpb.log`, `gauntlet_vtc.log`; `src/regimes/mpb.py`, `src/regimes/vtc.py` (both match KB-93 pre-registration line-for-line — trend/ER/thrust/CLV gates, SL bands, %-capped TP, dedupe/cooldown all as specified); `analysis/real_trades/ztt_sim.py` (fill/cost engine, unchanged from prior canon); `src/regimes/ztt_costs.py:26-34` (flat-$ session-gated cost, no price scaling); year-price check via `data/cache/oanda_gold_10y_10m.csv` (mean Close by year 2016→2026: 1280→1258→1269→1393→1772→1799→1801→1943→2389→3443→4767).
**Confidence:** High on "KILL deserved" and "session effect corroborates prior canon." Medium-high on "year trend is a cost-model artifact" (mechanically demonstrated but not itself gauntlet-gated).
**Implications:** Council should record MPB and VTC as KILL at N3 (N4-N8 never ran — correctly, per the gauntlet's halt-on-first-kill design). No harness bug found; no direction-only or session-only rescue exists (both fail everywhere). Flag `ztt_costs.py`'s flat-$ model as a latent confound for ANY future Gold candidate spanning multiple price eras — a %-of-price cost model (or per-era cost recalibration) would be a fairer test and should be considered before the next candidate's pre-registration freezes its cost assumptions. The 16-20/20:00 UTC-loses-money pattern is now confirmed across three independent Gold mechanizations (SBRS 1H, MPB 10m, VTC 10m) — treat it as a structural Gold microstructure fact, not a strategy-specific filter.
